...
namespace SportFIT.Controllers
{
    public class EstadisticasController
    {
        ...
        public DataTable ObtenerReservasPorInstalacion(string pueblo)
        {
            ...
            {
                string query = @"SELECT i.nombre_instalacion AS Instalacion,
                                        COUNT(r.id_reserva) AS Total_Reservas
                                FROM Instalacion i JOIN Reserva r ON i.id_instalacion = r.id_instalacion
                                                   JOIN Pueblo P on i.id_pueblo = P.id_pueblo
                                WHERE p.nombre_pueblo = @NombrePueblo
                                GROUP BY i.nombre_instalacion";
        ...
        public DataTable ObtenerPorcentajesReservas(string pueblo)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                string query = @"SELECT u.nombre_usuario AS Usuario,
                                        COUNT(*) AS Total_Reservas,
                                        ROUND(COUNT(*) * 100.0 / SUM(COUNT(*)) OVER (), 2) AS Porcentaje_Reservas
                                 FROM Usuario u JOIN Reserva r ON u.id_usuario = r.id_usuario
        ...
        public DataTable TendenciaReservasMes(string pueblo)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                string query = @"SELECT YEAR(r.fecha_reserva) AS Anio,
                                      DATENAME(MONTH, r.fecha_reserva) AS Mes,
                                      MONTH(r.fecha_reserva) AS NumeroMes,
                                      COUNT(*) AS Total_Reservas
                                 FROM Reserva r JOIN Instalacion i ON r.id_instalacion = i.id_instalacion
                                                JOIN Pueblo p ON i.id_pueblo = p.id_pueblo
                                 WHERE p.nombre_pueblo = @NombrePueblo
                                 GROUP BY YEAR(r.fecha_reserva),
                                          DATENAME(MONTH, r.fecha_reserva),
                                          MONTH(r.fecha_reserva)
                                 ORDER BY Anio, NumeroMes";
